...
export function registerMockEndpoint<T>(
    ...
    const handler: MockHttpRequestHandler<T> = {
        ...handler_ops,
        callback: handler_ops.callback || (a => a.metadata),
        path_parts,
 ...
 */
function mock_request(handler: MockHttpRequestHandler, request: MockHttpRequest) {
    ...
    return from([result]).pipe(concatMap(item => of(item).pipe(delay(Math.max(200, delay_time)))));
}
